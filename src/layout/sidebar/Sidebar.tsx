...
export const Sidebar: React.FC = () => {
    const [onSidebarItem, setOnSidebarItem] = useState(false);

    return (
        <S.Sidebar>
            <MenuBurger />
            <S.SidebarList>
                {items.map(({ title, href, icon, viewBoxIcon }, index) => <S.SidebarItem key={index} active={onSidebarItem} onClick={() => { setOnSidebarItem(onSidebarItem) }}> <S.SidebarLink to={href}>
